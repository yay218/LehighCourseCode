...
int main(int argc, char **argv)
{
	...
	if(student_check)
	{
		...
		if((*student.name1 == '\0') || (*student.id1 == '\0'))
		{
			printf("ERROR.	You must fill in all student fields!\n");
			exit(1);
		}
		else
		{
			printf("Student:%s:%s\n", student.name1, student.id1);
		}
	}
...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	if(fscanf(tracefile, "%d", &(trace->sugg_heapsize)) < 0)
	{
		/* not used */
		unix_error("Failed to read sugg_heapsize");
	}
	if(fscanf(tracefile, "%d", &(trace->num_ids)) < 0)
	{
		unix_error("Failed to read num_ids");
	}
	if(fscanf(tracefile, "%d", &(trace->num_ops)) < 0)
	{
		unix_error("Failed to read num_ops");
	}
	if(fscanf(tracefile, "%d", &(trace->weight)) < 0)
	{
		/* not used */
		unix_error("Failed to read weight");
	}
	...
	while(fscanf(tracefile, "%s", type) != EOF)
	{
		...
		{
		case 'a':
			if(fscanf(tracefile, "%u %u", &index, &size) < 0)
			{
				unix_error("Failed to read index/size of type 'a'");
			}
		...
		case 'r':
			if(fscanf(tracefile, "%u %u", &index, &size) < 0)
			{
				unix_error("Failed to read index/size of type 'r'");
			}
			...
			break;
		case 'f':
			if(fscanf(tracefile, "%ud", &index) < 0)
			{
				unix_error("Failed to read inex of type 'f'");
			}
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges)
{
	...
	for(unsigned int i = 0; i < trace->num_ops; i++)
	{
...
static double eval_mm_util(trace_t *trace, int tracenum, range_t **ranges)
{
	...
	for(unsigned int i = 0; i < trace->num_ops; i++)
	{
		...
		case traceop_t::ALLOC: /* mm_alloc */
			...
			/* Keep track of current total size of all allocated blocks */
			total_size += size;
