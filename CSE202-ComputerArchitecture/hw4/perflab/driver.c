 ...
 * program using the add_flip_function() and add_smooth_function()
 * functions.
 * 
...
static int test_dim_flip[] = {64, 96, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536};
static int test_dim_smooth[] = {32, 64, 96, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536};

/* Baseline CPEs (see config.h) */
static double flip_baseline_cpes[] = {R64, R96, R128, R256, R512, R1024, R2048, R4096, R8192, R16384, R32768, R65536};
static double smooth_baseline_cpes[] = {S32, S64, S96, S128, S256, S512, S1024, S2048, S4096, S8192, S16384, S32768, S65536};
...
/* Keep track of the best flip and smooth score for grading */
...
static int check_smooth(int dim) {
    ...
    for(i = 0; i < dim; i++)
	{
		for(j = 0; j < dim; j++)
		{
			struct pixel_t smoothed = check_average(dim, i, j, orig);
...
void test_smooth(int bench_index)
{
    ...
    /* Compute speedup */
    {
		...
		for(i = 0; i < DIM_CNT; i++)
		{
			...
			{
				ratio = smooth_baseline_cpes[i] / benchmarks_smooth[bench_index].cpes[i];
		    }
		    else
			{
				printf("Fatal Error: Non-positive CPE value...\n");
...
void usage(char *progname)
{
    ...
    fprintf(stderr, "  -g         Autograder mode: checks only flip() and smooth()\n");
...
int main(int argc, char *argv[])
{
    ...
    while((c = getopt(argc, argv, "tgqf:d:s:h")) != -1)
	{
		...
		case 's': /* seed for random number generator (hidden flag) */
		    seed = atoi(optarg);
		    break;
		case 'g': /* autograder mode (checks only flip() and smooth()) */
		    autograder = 1;
		    break;
		case 'q':
		    quit_after_dump = 1;
     ...
     * the file to determine the versions of flip and smooth to test
     */
